...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('createAppointmentSpecialtyId').addEventListener('change', function() {
        ...
        const professionalSelect = document.getElementById('createAppointmentProfessionalId');
        
        // Se "Geral / Todas" (valor vazio) for selecionado, carrega todos os profissionais
        // Se uma especialidade específica for selecionada, filtra por ela
        // Se nenhuma opção válida, desabilita
        if (this.value === '') {
            // "Geral / Todas" - carrega todos os profissionais sem filtro
            loadProfessionalsForSelect(null);
        } else if (this.value) {
            // Especialidade específica - filtra profissionais
    ...
    if (createAppointmentModal) {
        createAppointmentModal.addEventListener('hidden.bs.modal', function() {
            // Reseta o select de profissional
            const professionalSelect = document.getElementById('createAppointmentProfessionalId');
            professionalSelect.disabled = true;
            professionalSelect.innerHTML = '<option value="">Selecione uma especialidade primeiro';
        });
    }
...
async function loadProfessionalsForSelect(specialtyId = null) {
    try {
        ...
        const response = await apiRequest(url);
        // A API retorna { data: { professionals: [...], count: ... } }
        ...
        if (createSelect) {
            ...
            if (professionals.length === 0) {
                ...
                option.value = '';
                option.textContent = 'Nenhum profissional encontrado para esta especialidade';
        ...
        const createSelect = document.getElementById('createAppointmentProfessionalId');
        if (createSelect) {
            createSelect.innerHTML = '<option value="">Erro ao carregar profissionais</option>';
            createSelect.disabled = true;
        }
    }
}
